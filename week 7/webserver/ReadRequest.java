...
public class ReadRequest {
	...
	private static void handleConnection(Socket connection) {
		...
		try {
			...
			//read HTTP request -- the request comes in on first line,
			//and is of the form: Get <filename> HTTP/1.x
	...
	/**
	 * Send an HTTP Error header to the client
	 * The first line is a status message from the server to the client.
	 * The second line holds the connection type of the document
	 * The fourth line holds more information of the error.
	 */
	...
	private static void sendErrorResponse(int errorCode){
		switch (errorCode){
			case 404:
				...
				out.println("<html><head><title>Error</title></head><body><h2><Error: 404 Not Found</h2><p>The resource that you registered does not exist on this server.</p></body></html>");
				break;
			case 501:
	...
	private static String getMimeType(String fileName) {
		...
		else if (ext.equals("jpeg")) return "text/jpeg";
		else if (ext.equals("png")) return "text/png";
		else if (ext.equals("gif")) return "text/gif";
		else if (ext.equals("ico")) return "text/x-ico";
		else if (ext.equals("class")) return "text/java-vm";
		else if (ext.equals("jar")) return "text/java-archive";
		else if (ext.equals("zip")) return "text/zip";
		else if (ext.equals("xml")) return "text/xml";
		else if (ext.equals("xhtml")) return "text/xhtml + xml";
